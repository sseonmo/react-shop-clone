...
export function getCartItems(cartItems, userCart) {
	const request = axios
		...
		.then((response) => {
			// CartItem들에 해당하는 정보들을 Product Collection에서 가져온후
			// Quantity 정보를 넣어준다.
...
export function removeCartItem(prodcutId) {
	const request = axios.get(`${USER_SERVER}/removeFromCart?id=${prodcutId}`)
		.then((response) => {
            console.log('removeCartItem', response.data);
            // prodcutInfo, cart 정보를 조합해서 CartDetail 정보를 만든다.

            response.data.cart.forEach( item => {
                response.data.productInfo.forEach((product, index) => {
                    if (item.id === product.id) {
                        // prodect.quantity = item.quantity;
